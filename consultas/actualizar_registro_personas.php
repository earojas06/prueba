<?php 
	...
	$cedula = $_POST['cedula'];
	$casa = $_POST['casa'];
    $consulta = $conexion -> query("SELECT arrendatarios.* , casas.* FROM arrendatarios, casas, r_casa_arrendatario WHERE arrendatarios.cedula = '$cedula' AND casas.nombre = '$casa' " );  
    $consulta2 = $conexion -> query("SELECT propietarios.* , casas.* FROM propietarios, casas, r_casa_propietarios WHERE propietarios.cedula = '$cedula' AND casas.nombre = '$casa' " );  
    if ($res = mysqli_fetch_array($consulta)) {
        ...
        }else if ($res = mysqli_fetch_array($consulta2)) {
        	$identificacion = $res[0];
        	$nombre = $res[1];
        	$telefono = $res[2];
        	$genero = $res[3];
...
<head>
	...
	<script type="text/javascript" src="../js/validar_actualizar_personas.js"></script>
		...
		<div class="formulario_contenedor">
			<form method="POST">
				...
				<div class="cajas">
					...
					<div class="caja_texto">
						<select name="rol" id="rol" onChange="Mostrar(this.form)" disabled="false" >
				...
				<div class="cajas">
					...
					<div class="caja_texto">
					<select name="casa" id="casa" disabled="false">
						<optgroup label="casa"></optgroup>
						<option value="<?php echo $id_casa; ?>"><?php echo $casa_r; ?></option>
						<?php
				...
				<div class="botones2">
					<input type="button" name="actualizar"  class="e-botones2 actualizar" value="Actualizar" id="actualizar" name="actualizar" id="actualizar">
